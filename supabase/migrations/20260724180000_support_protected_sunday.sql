-- Allow a reviewed no-study planning draft to publish a protected recovery day
-- without manufacturing a placeholder assignment.
...
begin
  ...
  v_day_type := coalesce(nullif(selected_draft.draft ->> 'dayType', ''), 'normal');
  v_task_count := jsonb_array_length(selected_draft.draft -> 'tasks');

  if v_day_type = 'no-study' then
    if v_task_count <> 0 then
      raise exception 'A no-study day cannot contain assignments';
    end if;
  elsif v_task_count < 1 or v_task_count > 8 then
    raise exception 'A study day must contain between 1 and 8 assignments';
  end if;
  ...
  for v_task in select value from jsonb_array_elements(selected_draft.draft -> 'tasks') loop
    if coalesce(v_task ->> 'title', '') = ''
       ...
       or coalesce(v_task ->> 'category', '') not in ('Learn', 'Drill', 'Review', 'Test strategy', 'Practice test', 'Reading')
...
comment on function public.publish_planning_draft(uuid) is
  'Atomically replaces an unstarted study day or publishes a protected no-study day.';
