...
export function TaskCard({ task, completed, onToggle, compact = false, studentId, skills = [], drillResults = [], practiceTestResult, onResultSaved }: TaskCardProps) {
  ...
  const mixedDrill = task.category === 'Drill' && task.skillIds.length > 1
  const drillCorrect = drillResults.reduce((sum, result) => sum + result.correct, 0)
  const drillAttempted = drillResults.reduce((sum, result) => sum + result.attempted, 0)
  const hasResult = task.category === 'Practice test' ? Boolean(practiceTestResult) : drillResults.length > 0
          ...
          <div className="task-card__evidence">
            {hasResult ? (
              <div className="task-result-summary">
                <span className="task-result-chip"><Check size={13} /> {task.category === 'Practice test' && practiceTestResult
                  ? `Score ${practiceTestResult.totalScore}`
                  : `${drillCorrect}/${drillAttempted} · ${drillAttempted ? Math.round((drillCorrect / drillAttempted) * 100) : 0}%`}</span>
                {task.category === 'Drill' && (
      ...
      {showResultForm && studentId && onResultSaved && (
        <div className="task-card__result-form" id={resultFormId}>
          {task.category === 'Practice test'
            ...
            : mixedDrill
              ? <MixedDrillResultForm studentId={studentId} task={task} skills={skills} existingResults={drillResults} onSaved={() => { setShowResultForm(false); onResultSaved() }} onCancel={() => setShowResultForm(false)} />
              : <DrillResultForm studentId={studentId} task={task} skills={skills} existingResult={drillResults[0]} onSaved={() => { setShowResultForm(false); onResultSaved() }} onCancel={() => setShowResultForm(false)} />}
        </div>
      )}
    </article>
  )
