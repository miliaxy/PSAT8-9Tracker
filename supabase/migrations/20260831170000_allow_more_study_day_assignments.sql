-- Allow a parent-reviewed study day to contain more than eight focused assignments.
-- The parent-approved time limit remains the controlling capacity guardrail.
...
begin
  ...
  if v_day_type = 'no-study' and v_task_count <> 0 then
...
comment on function public.publish_planning_draft(uuid) is
  'Atomically replaces an unstarted study day or publishes a protected no-study day without an assignment-count cap.';
