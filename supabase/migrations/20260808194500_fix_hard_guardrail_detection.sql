-- Treat Hard as assigned only when the plan gives a Hard question count or an
-- explicit Hard difficulty. Explanatory wording such as "Hard remains locked"
-- must not trigger the mastery gate.
...
begin
  ...
  loop
    ...
    assigns_hard := task_text ~* '[0-9]+[[:space:]]+Hard([[:space:]]+questions?)?'
      or task_text ~* 'Difficulty[[:space:]]*:[[:space:]]*Hard';
    ...
    if jsonb_typeof(task -> 'skillIds') = 'array' then
      ...
      loop
        ...
        if task_category = 'Drill' and assigns_hard then
